...
from ws_chip_trace_generator import ws_chip_trace_generator
from constraint_checker import check_constriant


INT_32_MAX = 2147483647
# ----------------------------------------------------------------------
# PE testbench class definition
# ----------------------------------------------------------------------
class ws_chip_tb(nnsimTestBench):
    ...
    def configure(self):
        ...
        # ====================================================================================================
        (self.mapping, self.shape, self.ifmap_data, self.weights_data, self.bias_data) = \
        ...
        # -----------------------------------------------------------------------
        config             = {}
        config['mapping']  = self.mapping.copy()
        config['shape']    = self.shape.copy()
        shape_mapping_info = {'shape': self.shape.copy(), 'mapping': self.mapping.copy()}
        
        #===========================================================================
        # WeightsGLB configuration
        #===========================================================================       
        # the first fill port starts at physical address 0
        # the first drain port starts at physiacl address 0
        # two ports in this configuration both have full access to the memory
        weights_glb_config     = {'LM':                  [{'base': 0, 'bound': self.arch['depth']['WeightsGLB']-1}],\
                                  'lm_sram_map':         [{'drain': 0, 'fill': 1}],\
                                  'shape_mapping_info':  shape_mapping_info}
        config['WeightsGLB']   = weights_glb_config
        
        #===========================================================================
        # IFmapGLB configuration
        #===========================================================================                          
        ifmap_glb_config        = {'LM':                  [{'base': 0, 'bound': self.arch['depth']['IFmapGLB'] - 1}],\
                                   'lm_sram_map':         [{'drain': 0, 'fill': 1}],\
                                   'shape_mapping_info':  shape_mapping_info}

        config['IFmapGLB'] = ifmap_glb_config
        
        #===========================================================================
        # PsumGLB configuration
        #===========================================================================                          
        psum_glb_config        = { 'LM':                  [{'base': 0, 'bound': self.arch['depth']['PsumGLB'] - 1}],\
                                   'lm_sram_map':         [{'drain': 0, 'fill': 1, 'update': 1}],\
                                   'shape_mapping_info':  shape_mapping_info}

        config['PsumGLB'] = psum_glb_config       
        ...
        config['PE'] = { 'WeightsSp':            buffer_config,\
                         'shape_mapping_info':   shape_mapping_info}     
    ...
    def tick(self):
        ...
        # ------------------------- weights input trace ----------------------------------------------
        if self.weights_in_chn[0].vacancy():
            if self.weights_itrace_idx + self.WGLB_width -1 < len(self.weights_itrace):
                if not np.isnan(self.weights_itrace[self.weights_itrace_idx]):
                    weight_data_to_send = [ d for d in self.weights_itrace[self.weights_itrace_idx: self.weights_itrace_idx+ self.WGLB_width]]
                    self.weights_in_chn[0].push(weight_data_to_send)
                    self.weights_itrace_idx += self.WGLB_width
            else:
                self.weights_done = True
        # ------------------------- ifmap input trace ------------------------------------------------    
        if self.ifmap_chn[0].vacancy():
            if self.ifmap_itrace_idx + self.IGLB_width -1 < len(self.ifmap_itrace):
                if not np.isnan(self.ifmap_itrace[self.ifmap_itrace_idx]):
                    ifmap_data_to_send = [ d for d in self.ifmap_itrace[self.ifmap_itrace_idx: self.ifmap_itrace_idx+ self.IGLB_width]]
                    self.ifmap_chn[0].push(ifmap_data_to_send)
                    self.ifmap_itrace_idx += self.IGLB_width
            else:
                self.ifmap_done = True  
#        else:
#            print('no space')
                
        if self.psum_in_chn[0].vacancy():
            if self.psum_itrace_idx + self.PGLB_width -1 < len(self.psum_itrace):
                if not np.isnan(self.psum_itrace[self.psum_itrace_idx]):
                    psum_data_to_send = [ d for d in self.psum_itrace[self.psum_itrace_idx: self.psum_itrace_idx+ self.PGLB_width]]
                    self.psum_in_chn[0].push(psum_data_to_send)
                    self.psum_itrace_idx += self.PGLB_width
            else:
                self.psum_done = True  
#        
        # -------------------------------------------------------------------------------------------
